  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Total Tasks" value={stats?.totalTasks ?? '--'} accent="text-ink" />
        <StatCard label="In Progress" value={stats?.statusCounts?.['In Progress'] ?? '--'} accent="text-ocean" />
        <StatCard label="Done" value={stats?.statusCounts?.Done ?? '--'} accent="text-mint" />
        <StatCard label="Overdue" value={stats?.overdueTasks ?? '--'} accent="text-rose" />
      </section>
